...
const getFromStorage = <T>(key: string, defaultValue: T): T => {
  return getFromLocalStorage<T>(key, defaultValue)
}
...
export function getAlterEgos(): AlterEgoWithCategories[] {
  try {
    // Get alter egos
    const alterEgos: AlterEgo[] = getFromStorage(ALTER_EGOS_KEY, [])

    // Get category mappings
    const categoryMappings: AlterEgoCategoryMapping[] = getFromStorage(ALTER_EGO_CATEGORY_MAPPINGS_KEY, [])

    // Get categories
    const categories: AlterEgoCategory[] = getFromStorage(ALTER_EGO_CATEGORIES_KEY, DEFAULT_CATEGORIES)

    // Combine alter egos with their categories
    return alterEgos.map((alterEgo) => {
      const mappings = categoryMappings.filter((mapping) => mapping.alter_ego_id === alterEgo.id)
      const categoryIds = mappings.map((mapping) => mapping.category_id)
      const alterEgoCategories = categories.filter((category) => categoryIds.includes(category.id))

      return {
        ...alterEgo,
        categories: alterEgoCategories,
      }
    })
  } catch (error) {
...
export function deleteAlterEgo(id: string): void {
  try {
    // Get existing data
    const alterEgos: AlterEgo[] = getFromStorage(ALTER_EGOS_KEY, [])
    const alterEgoCategoryMappings: AlterEgoCategoryMapping[] = getFromStorage(ALTER_EGO_CATEGORY_MAPPINGS_KEY, [])
    const alterEgoPrompts: AlterEgoPrompt[] = getFromStorage(ALTER_EGO_PROMPTS_KEY, [])
    ...
    // Save to localStorage
    setToStorage(ALTER_EGOS_KEY, updatedAlterEgos)
    setToStorage(ALTER_EGO_CATEGORY_MAPPINGS_KEY, updatedCategoryMappings)
    setToStorage(ALTER_EGO_PROMPTS_KEY, updatedPromptRelations)
  } catch (error) {
...
export function getCategories(): AlterEgoCategory[] {
  try {
    const storedCategories = getFromStorage(ALTER_EGO_CATEGORIES_KEY, null)
    if (!storedCategories) {
      // Initialize with default categories
      setToStorage(ALTER_EGO_CATEGORIES_KEY, DEFAULT_CATEGORIES)
      return DEFAULT_CATEGORIES
    }

    // Sort by position
    const categories: AlterEgoCategory[] = storedCategories
    return categories.sort((a: AlterEgoCategory, b: AlterEgoCategory) => a.position - b.position)
  } catch (error) {
